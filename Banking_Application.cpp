...
class bms{
    ...
    public: // data abstraction
        bms(){ //constructor
            ...
            gotoxy(26, 4);
            design(15, 177);
            cout<<" WELCOME TO SMC BANKING SYSTEM ";
            design(15, 177);
            gotoxy(50, 8);
            cout<<"ACCOUNT TYPE";
            gotoxy(44, 10);
    ...
    if (m == 1) {
        ...
        // CHECK FROM FILE instead of hardcoded
        record rec;
        ifstream f;
        f.open("record.bin", ios::in | ios::binary);
        while (f.read(reinterpret_cast<char*>(&rec), sizeof(rec))) {
            if (strcmp(id, rec.email) == 0 && strcmp(password, rec.citiz) == 0) {
                f.close();
                gotoxy(35, 17);
                design(48, 45);
                gotoxy(38, 16);
